...
| `GaussVonMisesGradientExample.main(args: Array[String]): Unit` | Empty arguments; same documentation package | Checks local log-density sensitivity, directional approximation and the squared-score gradient identity; no model or RNG | `GaussVonMisesGradientExample.main(Array.empty[String])` |
| `GaussVonMisesQuadratureExample.main(args: Array[String]): Unit` | Empty arguments; same documentation package | Compares sparse scalar/vector expectations with analytic and Monte Carlo answers, counts callbacks and demonstrates a signed-rule failure | `GaussVonMisesQuadratureExample.main(Array.empty[String])` |
| `GaussVonMisesTensorQuadratureExample.main(args: Array[String]): Unit` | Empty arguments; same documentation package | Compares positive tensor estimates at increasing orders with analytic integrals, reports errors/callback counts and checks coupled vector moments | `GaussVonMisesTensorQuadratureExample.main(Array.empty[String])` |
| `GaussVonMisesQuadratureComparisonExample.main(args: Array[String]): Unit` | Empty arguments; same documentation package | Checks scalar/vector four-corner order diagnostics, total callback budgets and an analytic false-agreement counterexample | `GaussVonMisesQuadratureComparisonExample.main(Array.empty[String])` |
| `GaussVonMisesBhattacharyyaExample.main(args: Array[String]): Unit` | Empty arguments; same documentation package | Checks exact and curved distances, unresolved outcomes and a six-dimensional matched-error comparison with tensor work counts | `GaussVonMisesBhattacharyyaExample.main(Array.empty[String])` |
