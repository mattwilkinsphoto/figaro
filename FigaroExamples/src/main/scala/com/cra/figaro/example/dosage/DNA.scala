...
object DNA {
  ...
  def nucToElement(n: String) = if (symbolToN(n).length > 1) Uniform(symbolToN(n)*) else Constant(symbolToN(n)(0))

  def codonToElement(c: List[String]): Element[String] = {
    val clist = c.map { t =>
      val listElem = t.map(s => nucToElement(s.toString()))
      Apply(Inject(listElem*), (s: List[String]) => s reduce (_ + _))
    }
    if (clist.size > 1) Uniform(clist*) else clist(0)
  }
  ...
  def genDNA(seq: AminoAcidSequence): Element[NucleotideSequence] = {
    ...
    val concat = Inject(codons*)
    Apply(concat, (s: List[String]) => (s).foldLeft(NucleotideSequence(""))(_ + NucleotideSequence(_)))
  }
