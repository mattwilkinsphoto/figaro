...
Modernization status: this branch builds with JDK 17, sbt 2.0.8, and Scala 2.13.18. It preserves the original `com.cra.figaro` API namespace while replacing obsolete JSci and Akka runtime dependencies, removing unused legacy dependencies, and adding focused regression and reproducible-artifact CI gates. See [MODERNIZATION.md](MODERNIZATION.md) and [DEPENDENCIES.md](DEPENDENCIES.md) for decisions and evidence.
...
sbt 2 writes Figaro artifacts under `target/out/jvm/scala-2.13.18/figaro/`. Supply multiple commands as one quoted, semicolon-separated string. The Scala 3 compiler used internally by sbt does not change the library's Scala 2.13 binary compatibility. A Scala 3 library migration is a separate stage.
