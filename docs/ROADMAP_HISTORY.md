# Figaro roadmap history through 6.1.0

This is a historical record of the modernization and capability-expansion program,
not the active work queue. The [current roadmap](../ROADMAP.md) is authoritative for
priorities; the [6.1 release record](RELEASE_6_1.md) and [Central publication record](MAVEN_CENTRAL.md)
define the consumable baseline.

Milestone test counts, branch names, source-only release statements and words such as
"next", "future" and "proposed" below describe their original checkpoints. Many of
those follow-ons subsequently shipped. They do not mean that 6.1.0 is unpublished
or that a completed milestone must be repeated. Numerical limitations remain valid
unless a later linked contract explicitly supersedes them. Retaining this record
does not turn a research prototype into a released API.

## Current baseline

The integrated feature release is **6.1.0**, beyond the 6.0 modernization
baseline. Its [release record](RELEASE_6_1.md) covers consumable release
...
on main. See [acceptance evidence](CORE_PERFORMANCE_ACCEPTANCE.md),
[release notes](RELEASE_NOTES.md) and [migration limitations](MIGRATION.md).
...
1. [Legacy-distribution consolidation](LEGACY_DISTRIBUTION_CONTRACTS.md)
...
2. [Covariance modeling](COVARIANCE_PRIORS.md) (modern.20, integrated on main at CI-verified `59f63f57`): LKJ correlation priors and inverse-Wishart, with explicit
   correlation/scale/covariance semantics and independent numerical checks.
3. [Query-aware rare-event proposals](RARE_EVENT_PROPOSALS.md) (modern.21, integrated on main at CI-verified `751579be`): literature-led design, discarded pilot costs
   included in [complete comparisons](RARE_EVENT_ACCEPTANCE.md), held-out accuracy
...
See [acceptance evidence](CAPABILITY_EXPANSION_ACCEPTANCE.md).

1. [Event-weighted multi-component Gaussian fitting](WEIGHTED_RARE_EVENT_MIXTURES.md): explicit component count, weighted
...
2. [Broader restricted graph execution](STATIC_GRAPH_PROPOSALS.md): explicit frozen joint-root proposals in the
...
3. [Reusable constructions](EXTENDED_CONSTRUCTIONS.md): caller-specified monotone transforms, half-infinite
...
1. [Observation semantics](OBSERVATION_MODELS.md): exact/interval/censored/rounded
   likelihoods and explicit hierarchical measurement-error factors.
2. [Mixed scalar measures](MIXED_MEASURES.md): atoms plus continuous slabs,
   clipping/rectification, observation adapters and full-law KL/Bhattacharyya.
3. [GVM mixtures](GVM_MIXTURES.md): fixed mixture kernels, responsibilities,
   normalized vector charts and information comparisons; test-only fitting comparison.
4. [Continuous copulas](COPULAS.md): Gaussian and Student-t latent dependence,
   scalar marginal transforms, graph evidence and appropriate information measures.

[Acceptance and research evidence](MODELING_CAPABILITIES_ACCEPTANCE.md) separates
...
The [GVM/GMM research plan](GVM_MIXTURE_RESEARCH_PLAN.md) retains that application boundary.

The approved [three-stage follow-on](INFERENCE_NEXT_STAGES.md) now has
[end-to-end cost evidence](GRAPH_COST_ACCEPTANCE.md), explicit
[conditional/joint proposals](JOINT_PROPOSALS.md), a full-rank
[multivariate Student t](MULTIVARIATE_STUDENT_T.md), and cross-family
[Monte Carlo KL/Bhattacharyya/MI](MONTE_CARLO_INFORMATION.md).
...
coverage](BOUNDED_IID_RELIABILITY.md), with explicit assumptions, outward
arithmetic and work budgets. The [restricted graph ownership design](OWNED_GRAPH_EXECUTION_DESIGN.md)
is documented. Modern.16 adds
[variance-adaptive bounded confidence sequences](ADAPTIVE_BOUNDED_PRECISION.md),
including total-cost counterexamples. Modern.17 implements the first
[restricted static executor](STATIC_GRAPH_EXECUTION.md), with semantic,
concurrency and scaling evidence. Modern.18 adds selected [joint-count, extreme-value,
matrix and directional representatives](DISTRIBUTION_BREADTH.md): multinomial,
...
Reliability interlude: [statistical validation](STATISTICAL_VALIDATION.md) and
[scientific RNG backends](RNG_ASSESSMENT.md) are implemented on the
...
concentrated posteriors at matched accuracy. [Versioned stream allocation and Philox](RNG_STREAMS.md)
...
The opt-in [purpose selector](RNG_SELECTION.md) now resolves versioned presets,
...
The opt-in [inference-health assessment](INFERENCE_HEALTH.md) adds explicit
...
[Pilot-fitted defensive importance research](DEFENSIVE_IMPORTANCE_RESEARCH.md)
...
coverage batch. The additive [public frozen-proposal API](VECTOR_IMPORTANCE.md)
...
The [6,600-trial calibration assessment](IMPORTANCE_CALIBRATION.md) is integrated
...
The [multi-component proposal fitter](MIXTURE_PROPOSALS.md) is integrated on
...
The [owned graph proposal bridge](GRAPH_PROPOSALS.md) is integrated on main as
...
implemented and locally validated; see its [guide and evidence](VON_MISES.md).
The [inventory](DISTRIBUTION_SUPPORT.md) distinguishes
...
| D2: linear-angular joint models (`DIST-02`) | Through mutual information, integrated with D3 at CI-verified `e78a6f0e` | Fixed-kernel Gauss-von Mises on a real vector plus one angle, built on D1 | [Scope approval, evidence and boundaries](GAUSS_VON_MISES.md); each extension retains its own CI/integration gate and separate future review for report ingestion/fusion/filtering/propagation |
| D3: common missing scalar/count families | Complete on main; source CI verified at `e78a6f0e` | All nine representatives: Student t/Cauchy/Laplace; negative binomial/hypergeometric; lognormal/Weibull; triangular/Kumaraswamy; appropriate information measures | [Milestone evidence](COMMON_DISTRIBUTIONS_ACCEPTANCE.md): density/tail oracles, observation and MCMC checks, isolated multi-chain determinism, divergence references and independent published consumer |
| D4: reusable constructions (`DIST-10`) | Initial milestone complete on main; source CI verified at `79615111` | Affine/exp transformations, finite truncation, scalar mixtures, full-covariance Gaussian mixture models, hurdle/zero-inflated counts; Gaussian KL/Bhattacharyya and partition MI | [Guide](DISTRIBUTION_CONSTRUCTIONS.md) and [acceptance evidence](DISTRIBUTION_CONSTRUCTIONS_ACCEPTANCE.md): 362 modernization tests, independent oracles/consumer and clean CI reproducibility; no generic mixed-measure law; pilot-only fitting arrived in modern.12 and numerical mixture metrics in modern.14 |
| D5: multivariate, tail and matrix breadth | Multivariate t, five selected representatives and covariance priors integrated | Multinomial, GEV, GPD, restricted full-rank Wishart, inverse-Wishart, LKJ and S2 von Mises-Fisher; appropriate KL/Bhattacharyya and count-block MI | [Breadth](DISTRIBUTION_BREADTH.md) and [covariance verification](COVARIANCE_PRIORS.md); other directional dimensions and specialized variants remain wishlist |
...
1. Canonical residual/inverse, squared Mahalanobis-von-Mises score and analytic directed
   ...
   see [contracts and validation](GVM_DIAGNOSTICS.md). Exact Gaussian/Mahalanobis
...
2. Analytic circular/mixed moments and exact angle-given-linear conditional: integrated
   ...
   see [moments contracts](GVM_MOMENTS.md).
3. [Finite-concentration score calibration](GVM_SCORE_CALIBRATION.md): integrated
...
4. [Analytic state gradients](GVM_GRADIENTS.md): integrated on main at `141dcc15`,
...
5. [Third-order deterministic expectation quadrature](GVM_QUADRATURE.md): integrated
...
6. [Positive-weight tensor reference](GVM_TENSOR_QUADRATURE.md): integrated on main
...
7. [Budgeted order comparison](GVM_QUADRATURE_COMPARISON.md): integrated on main
...
8. [Bhattacharyya numerical-method assessment](GVM_BHATTACHARYYA_RESEARCH.md):
...
9. [Guarded Scala Bhattacharyya comparison](GVM_BHATTACHARYYA.md): integrated on main
   ...
   1e-6-nat error target. The follow-on [matched-accuracy timing study](GVM_BHATTACHARYYA_PERFORMANCE.md)
   ...
   The follow-on [concentration/cancellation grid](GVM_BHATTACHARYYA_RELIABILITY.md)
   ...
   production code changed. The [positive scalar integration assessment](GVM_BHATTACHARYYA_POSITIVE_RESEARCH.md)
   ...
   The follow-on [scalar Scala prototype](GVM_POSITIVE_SCALAR_PROTOTYPE.md)
   ...
   The implementation and tests now move into an [explicit public scalar API](GVM_SCALAR_BHATTACHARYYA.md)
   ...
   The [matched-accuracy scalar study](GVM_SCALAR_PERFORMANCE.md)
   ...
   The follow-on [audited-totals optimization](GVM_SCALAR_AUDITED_TOTALS.md) targets
   ...
   The [tail-radius assessment](GVM_SCALAR_TAIL_ASSESSMENT.md) now tests a stronger
   ...
   throughput. The [test-only JVM candidate](GVM_SCALAR_TAIL_JVM.md) now retains
   ...
   [Production integration](GVM_SCALAR_TAIL_PRODUCTION.md) now passes 319 distinct
...
10. [Linear-angular mutual information](GVM_MUTUAL_INFORMATION.md): implemented
   ...
   see [acceptance evidence](COMMON_DISTRIBUTIONS_ACCEPTANCE.md).
   The [cross-family metrics roadmap](INFORMATION_METRICS_ROADMAP.md) carries KL,
...
7. Assess information-metric support using the [cross-family checklist](INFORMATION_METRICS_ROADMAP.md).
...
Related: [wishlist](../WISHLIST.md), [distribution inventory](DISTRIBUTION_SUPPORT.md),
[GVM milestone](GAUSS_VON_MISES_PLAN.md), [engineering history](../MODERNIZATION.md).
