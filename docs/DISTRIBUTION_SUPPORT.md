...
| --- | --- | --- |
| ObservationLikelihood, ScalarObservation | [Observation semantics](OBSERVATION_MODELS.md) | Exact density versus interval/censoring/rounding mass and explicit graph factors; continuous kernels only |
| MixedScalarDistribution, MixedScalarElement, MixedScalarInformation | [Mixed measures](MIXED_MEASURES.md) | Finite atoms plus continuous slab, clipping/rectification and full-law KL/Bhattacharyya; no arbitrary mixed convolution or random atom-location likelihoods |
| GaussVonMisesMixtureDistribution, GaussVonMisesMixture | [GVM mixtures](GVM_MIXTURES.md) | Fixed normalized kernels, linear marginal, responsibilities, chart adapters, full-mixture numerical comparisons and label/state MI |
| GaussVonMisesMixtureFit, GaussVonMisesMixtureMutualInformation | [Fitting](GVM_MIXTURE_FITTING.md), [partition MI](GVM_MIXTURE_MI.md) | Bounded scalar-linear periodic fitting with explicit statuses; full vector/angle mixture MI with separate sampling/numerical diagnostics; no automatic component selection or higher-dimensional fitting |
| CopulaDistribution, ConditionalCopulaDistribution, CopulaElement | [Continuous copulas](COPULAS.md) | Full-rank Gaussian/t dependence with continuous marginals; normalized vector proposals, exact marginals, exact-coordinate partial evidence/conditioning and Gaussian partition MI; no mixed/count copulas or conditional interval integration |
...
| GaussVonMises, GaussVonMisesDistribution, LinearAngular | [Joint guide](GAUSS_VON_MISES.md) | Shipped in 6.1.0: fixed kernels, complete joint observations and documented diagnostics/information helpers; numerical and inference limits remain explicit |
