...
| D2: linear-angular joint models (`DIST-02`) | Kernel, KL/residuals and moments/conditionals on main at CI-verified `df5a7bf4`; finite-concentration score calibration locally validated, awaiting CI | Fixed-kernel Gauss-von Mises on a real vector plus one angle, built on D1 | [Scope approval, evidence and boundaries](docs/GAUSS_VON_MISES.md); each extension retains its own CI/integration gate and separate future review for report ingestion/fusion/filtering/propagation |
...
3. [Finite-concentration score calibration](docs/GVM_SCORE_CALIBRATION.md): implemented
   on the development branch, 14 focused tests and all 234 modernization regressions
   passing locally; remote CI and integration pending. Direct upper tails, quantiles,
   error estimates and modeled-coverage tests replace an assumed chi-square threshold.
4. Gradients and deterministic expectation quadrature follow separately, with their own
   accuracy and approximation contracts. Neither is implemented by score calibration.
5. Bhattacharyya divergence and linear-angular mutual information: lower-priority research
