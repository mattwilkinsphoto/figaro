 ...
 */
class MultiDecisionVariableElimination(universe: Universe, utilityNodes: List[Element[?]], targets: Element[?]*)
  ...
  protected def createAlg[T, U](decisionTarget: Decision[T, U],
    ...
    val ve = new ProbQueryVariableEliminationDecision[T, U](mv, utilityNodes, decisionTarget)(
      false,
      List(),
      (u: Universe, e: List[NamedEvidence[?]]) => () => ProbEvidenceSampler.computeProbEvidence(10000, e)(using u)) with OneTimeProbQueryDecision[T, U]
    ve
  }
}
...
object MultiDecisionVariableElimination {
   ...
   */
  def apply(utilityNodes: List[Element[?]], targets: Element[?]*)(implicit universe: Universe) = {
    new MultiDecisionVariableElimination(universe, utilityNodes, targets*)
  }
