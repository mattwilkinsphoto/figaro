...
class StructuredVEGibbsChooser(universe: Universe, scoreThreshold: Double, numSamples: Int, burnIn: Int, interval: Int, blockToSampler: Gibbs.BlockSamplerCreator, targets: Element[?]*)
...
object StructuredVEGibbsChooser {
   ...
   */
  def apply(scoreThreshold: Double, numSamples: Int, targets: Element[?]*) = {
    ...
    new StructuredVEGibbsChooser(targets(0).universe, scoreThreshold, numSamples, 0, 1, BlockSampler.default, targets*)
  }

  /**
   * Create a hybrid algorithm that chooses between variable elimination and Gibbs sampling on each subproblem.
   */
  def apply(scoreThreshold: Double, numSamples: Int, burnIn: Int, interval: Int, blockToSampler: Gibbs.BlockSamplerCreator, targets: Element[?]*) = {
    if (targets.isEmpty) throw new IllegalArgumentException("Cannot run VE/Gibbs with no targets")
    val universes = targets.map(_.universe).toSet
    if (universes.size > 1) throw new IllegalArgumentException("Cannot have targets in different universes")
    new StructuredVEGibbsChooser(targets(0).universe, scoreThreshold, numSamples, burnIn, interval, blockToSampler, targets*)
  }
