 ...
 */
case class UntypedScheme(first: () => Element[?], rest: Option[ProposalScheme]) extends ProposalScheme
 ...
 */
case class FinalScheme(element: () => Element[?]) extends ProposalScheme
...
object ProposalScheme {
   ...
   */
  def apply(elems: Element[?]*): ProposalScheme = {
    elems.toList match {
      case Nil => throw EmptyProposalScheme
      case List(elem) => FinalScheme(() => elem)
      case elem :: rest => UntypedScheme(() => elem, Some(apply(rest*)))
    }
  }
}
