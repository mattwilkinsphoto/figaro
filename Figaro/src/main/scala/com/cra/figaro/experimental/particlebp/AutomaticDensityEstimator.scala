...
class AutomaticDensityEstimator extends NormalKernelDensityEstimator {
  ...
  def getBandwidth(samples: List[(Double, Double)]): Double = {
    val mean = (samples).foldLeft(0.0)((c: Double, n: (Double, Double)) => c + n._1 * n._2)
    val std = math.sqrt((samples).foldLeft(0.0)((c: Double, n: (Double, Double)) => c + (n._2 - mean) * (n._2 - mean) * n._1))
