...
object DecisionUtil {
   ...
   */
  def getDecisionOrder(Decisions: Iterable[Element[?]], universe: Universe): List[List[Element[?]]] =
    universe.layers(Decisions)

  /**
   * Returns the utilities in a list of elements from a reference list of utilities.
   */
  def utilitiesInElems(Elems: List[Element[?]], U: List[Element[?]]): List[Element[?]] =
    Elems.intersect(U)

  /**
   * Computes the set of elements that the given list of utilities depends upon for computation.
   */
  def utilitiesUse(U: Iterable[Element[?]], universe: Universe): Set[Element[?]] =
    U.foldLeft(Set[Element[?]]())((s, u) => s union universe.uses(u)).toSet
   ...
   *  */
  def getPredDecisions(D: Element[?], Order: List[List[Element[?]]], universe: Universe) = {
   ...
   */
  def getSuccDecisions(D: Element[?], Order: List[List[Element[?]]], universe: Universe) = {
    val contain = Order.map(s => s.contains(D)).indexOf(true)
    val allD = Order.splitAt(contain)._2.flatten.diff(List(D)) intersect universe.usedBy(D).toList
    (allD).foldLeft(allD)((c, n) => c.diff(universe.usedBy(n).toList))
  }

  /**
   * Computes the relevant utility elements for a specific decision.
   */
  def getReleventUtil(D: Element[?], utilities: List[Element[?]], Order: List[List[Element[?]]], universe: Universe) = {
    val utilD = utilitiesInElems(universe.usedBy(D).toList, utilities)
    val succUtil = getSuccDecisions(D, Order, universe).foldLeft(Set[Element[?]]()) { (s, u) =>
   ...
   */
  def getElemsForDecision(D: Element[?], utilities: List[Element[?]], Order: List[List[Element[?]]], universe: Universe) = {
    val utilUses = getReleventUtil(D, utilities, Order, universe).foldLeft(Set[Element[?]]()) { (s, u) =>
      s union universe.uses(u).toSet union Set(u)
    }
    val predDecUses = getPredDecisions(D, Order, universe).foldLeft(Set[Element[?]]()) { (s, u) =>
      s union (u match {
    ...
    val succDecUsedBy = getSuccDecisions(D, Order, universe).foldLeft(Set[Element[?]]()) { (s, u) =>
      s union universe.usedBy(u).toSet
    }
    val succDecUses = getSuccDecisions(D, Order, universe).foldLeft(Set[Element[?]]()) { (s, u) =>
