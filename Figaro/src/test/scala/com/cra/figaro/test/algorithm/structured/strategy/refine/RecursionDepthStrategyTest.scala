/*
 * RecursionDepthStrategyTest.scala
 * Tests for strategies that use recursion depth.
 *
...
import com.cra.figaro.algorithm.structured.strategy.refine.RecursionDepthStrategy
import com.cra.figaro.algorithm.structured.strategy.solve.{ConstantStrategy, structuredRaising}
...
class RecursionDepthStrategyTest extends AnyWordSpec with Matchers {
  "A complete recursion depth strategy" should {
    "create ranges for all components" in {
      ...
      new RecursionDepthStrategy(pr, pr.targetComponents).execute()
    ...
    "preserve solutions to solved subproblems" in {
      ...
      new RecursionDepthStrategy(spr, spr.targetComponents).execute()
    ...
    "recursively mark problems as unsolved" when {
      ...
      "executed on a nested problem" in {
        Universe.createNew()
        val e1 = Flip(0.3)
        val e2 = If(e1, Select(0.1 -> 1, 0.9 -> 2), discrete.Uniform(3, 4))
        val cc = new ComponentCollection
      ...
      "executed directly on a nested problem already marked as unsolved" in {
        ...
        pr.solved = true
        pr.solution = dummySolution
        new RecursionDepthStrategy(c2.subproblems(true), pr.targetComponents).execute()
