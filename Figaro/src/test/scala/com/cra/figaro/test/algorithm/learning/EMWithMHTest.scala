...
class EMWithMHTest extends AnyWordSpec with PrivateMethodTester with Matchers {
  ...
  "Expectation Maximization with MetropolisHastings" when
    {
        ...
        "exit before reaching the maximum iterations" in {
            ...
            val algorithm = EMWithMH(terminationCriteria, 10000, b)(using universe)
            algorithm.start()
          ...
          "detect bias after a large enough number of trials" in
            {
              ...
              val algorithm = EMWithMH(2, 100000, b)(using universe)
              algorithm.start()
              ...
              "leave parameters having no observations unchanged" in
                {
                  ...
                  val algorithm = EMWithMH(2, 100000, d, b)(using universe)
                  algorithm.start()
              ...
              "correctly estimate all parameters with observations" in
                {
                  ...
                  val algorithm = EMWithMH(2, 100000, b,d)(using universe)
                  algorithm.start()
          ...
          def learner(parameters: Parameters): Algorithm = {
            parameters match {
              case ps: LearnableParameters => 
                EMWithMH(numEMIterations, 50000, ps.b1, ps.b2, ps.b3, ps.b4, ps.b5, ps.b6, ps.b7, ps.b8, ps.b9)(using parameters.universe)
