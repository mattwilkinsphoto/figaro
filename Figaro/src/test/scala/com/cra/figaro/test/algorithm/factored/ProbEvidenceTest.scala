...
class ProbEvidenceTest extends AnyWordSpec with Matchers {
  ...
  def sampleTest(prob: Double, evidence: List[NamedEvidence[?]], universe: Universe): Unit = {
    
    //universe.assertEvidence(evidence)
    //val alg = BeliefPropagation(100, universe.activeElements.toList:_*)
    //alg.start
    ProbEvidenceBeliefPropagation.computeProbEvidence(100, evidence)(using universe) should be(prob +- 0.01)
