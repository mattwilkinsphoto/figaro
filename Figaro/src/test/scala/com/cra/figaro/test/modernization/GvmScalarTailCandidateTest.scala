// Candidate reuses every public scalar contract/oracle test; public behavior is unchanged.
...
class GvmScalarTailCandidateTest extends AnyWordSpec with Matchers {
  import GvmScalarTailCandidate.{compare,Status}
  ...
  "The opt-in scalar positive GVM comparison" should {
    ...
    "keep work buffers independent across concurrent calls" in {
      ...
      try {
        val tasks=Vector.fill(8)(pool.submit(new Callable[GvmScalarTailCandidate.Result] {
          def call(): GvmScalarTailCandidate.Result = compare(p,q)
        }))
