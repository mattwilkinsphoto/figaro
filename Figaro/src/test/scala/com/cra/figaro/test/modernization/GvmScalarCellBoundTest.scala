...
class GvmScalarCellBoundTest extends AnyWordSpec with Matchers {
  ...
  "The test-only scalar cell bound" should {
    ...
    "handle zero, subnormal and extreme phases conservatively and reject invalid parameters" in {
      for(k <- Vector(0.0,java.lang.Double.MIN_VALUE,1e-100,50.0)) {
        val value=GvmScalarCellBound.lower(math.Pi,0,0,k,50)
        value.isFinite shouldBe true; value should be >= 0.0; value should be <= 1.0
      }
      GvmScalarCellBound.lower(Double.MaxValue,Double.MaxValue,Double.MaxValue,50,50) shouldBe 0.0
      intercept[IllegalArgumentException] { GvmScalarCellBound.lower(Double.NaN,0,0,1,1) }
      intercept[IllegalArgumentException] { GvmScalarCellBound.lower(0,0,0,-1,1) }
      intercept[IllegalArgumentException] { GvmScalarCellBound.lower(0,0,0,1,50.1) }
      intercept[IllegalArgumentException] { GvmScalarCellBound.lower(0,0,0,1,1,null) }
    }
    "retain cheap-case diagnostics and full tail accounting while reducing curved work" in {
      ...
      for(t <- Vector(1e-6,1e-8,1e-12); budget <- Vector(5,500,7470,9102,15000,50000)) {
        val r=GvmScalarTailCandidate.compare(p,q,t,budget)
        r.evaluations should be <= budget
        if(r.status == GvmScalarTailCandidate.Status.Estimated)
